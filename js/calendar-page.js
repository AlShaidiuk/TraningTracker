class CalendarPage {
    constructor() {
        ...
        this.plannedDates = [];         // массив строк 'YYYY-MM-DD'
        this.workoutStatuses = {};      // данные из workoutData/{date}
        this.templates = [];            // список шаблонов { id, name, exercises }
        ...
        this.currentExercises = [];     // для конструктора шаблона
    ...
    async init() {
        ...
        await this.loadWorkoutStatuses();   // загружаем статусы выполненных тренировок
        this.renderCalendar();
    ...
    /* ========== Работа с запланированными датами ========== */
    ...
    /* ========== Загрузка статусов тренировок ========== */
    async loadWorkoutStatuses() {
        try {
            const snap = await firebase.database().ref('workoutData').once('value');
            const data = snap.val() || {};
            this.workoutStatuses = data;   // { 'YYYY-MM-DD': { savedSets, completedSets, fullyCompleted } }
        } catch (e) {
    ...
    async createTemplate(templateData) {
        try {
            ...
            this.templates.push({ id: newRef.key, name: templateData.name, exercises: templateData.exercises });
            return newRef.key;
        } catch (error) {
    ...
    // Определяет, находится ли дата в прошлом (до начала сегодняшнего дня)
    isPastDay(dateStr) {
    ...
    renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (this.isPlanned(date)) {
                ...
                // Если день уже прошёл, определяем статус
                if (date < todayStart) {
                    const statusData = this.workoutStatuses[dateStr];
                    if (statusData) {
                        if (statusData.fullyCompleted === true) {
                            dayCell.classList.add('status-completed');
                            dayCell.innerHTML = '⭐';   // заменяем число на звёздочку
                        } else {
                            dayCell.classList.add('status-partial');
                            // число остаётся, но фон будет жёлтым
                        }
                    } else {
                        // Тренировка была запланирована, но нет никаких данных – пропуск
            ...
            // Выделение текущего выбранного дня
    ...
    async refreshDataAndRender() {
        await this.loadPlannedDatesFromDB();
        await this.loadWorkoutStatuses();   // обновляем и статусы
        if (this.selectedDate && !this.plannedDates.includes(this.selectedDate)) {
            if (this.selectedElement) {
                this.selectedElement.classList.remove('selected');
            }
    ...
    setupSettingsButton() {
        ...
        settingsBtn.addEventListener('click', () => {
            ...
            // Блокируем кнопку "Добавить тренировку" для прошедших дней
            const past = this.isPastDay(this.selectedDate);
            if (addBtn) {
                addBtn.disabled = past;
                addBtn.title = past ? 'Нельзя добавить тренировку на прошедшую дату' : '';
            }
